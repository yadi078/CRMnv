    ...
    <x-slot name="header">
        ...
        <div class="flex flex-wrap gap-2 ml-auto justify-end items-center">
            ...
            @if(isset($sale) && $sale && $contact->fichaPdfCompleta())
                @can('view', $sale)
                    <a href="{{ route('user.sales.ficha-pdf', $sale) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/15 text-white border border-white/35 hover:bg-white/25 text-sm font-semibold" title="Descargar ficha de inscripción en PDF">
