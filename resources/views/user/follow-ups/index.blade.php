<x-app-user-layout>
    <x-slot name="header">
        ...
        <div>
            ...
            <p class="page-header-card__subtitle">Alarmas y notas de seguimiento</p>
        </div>
        @can('follow-ups.create')
        <a href="{{ route('follow-ups.create') }}" class="btn-amber-app ml-auto">
            ...
            <div class="flex items-start gap-3 px-4 sm:px-5 py-3 hover:bg-white/5 transition-colors border-l-4 @if($followUp->completado) border-l-[#15803D] @elseif($followUp->estaVencido()) border-l-[#B91C1C] @else border-l-[#B45309] @endif">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm font-medium text-white">
                        @if($followUp->company)
                            Empresa: <a href="{{ \App\Support\CrmNavigation::withReturn(route('companies.show', $followUp->company)) }}" class="text-[#FFE600] hover:text-white">{{ $followUp->company->nombre_comercial }}</a>
                        @elseif($followUp->contact)
                            Contacto: <a href="{{ \App\Support\CrmNavigation::withReturn(route('contacts.show', $followUp->contact)) }}" class="text-[#FFE600] hover:text-white">{{ $followUp->contact->nombre_completo }}</a>
                        @else
                ...
                <div class="flex gap-2 flex-shrink-0">
                    <a href="{{ \App\Support\CrmNavigation::withReturn(route('follow-ups.show', $followUp)) }}" class="text-[#FFE600] hover:text-white inline-flex items-center gap-1 text-sm font-medium">
