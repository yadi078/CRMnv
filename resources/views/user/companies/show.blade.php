    ...
    <div class="space-y-8">
        ...
        <div class="panel-card-dark p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="p-4 rounded-xl bg-white/5 flex justify-between items-start panel-card-dark__row hover:bg-white/8">
                    <div>
                        <p class="font-medium text-white">{{ $contact->nombre_completo }}</p>
                        <p class="text-sm text-white/80">{{ $contact->puesto_de_trabajo ?? '-' }} · {{ $contact->email }}</p>
                    </div>
                    <a href="{{ route('contacts.show', $contact) }}" class="text-[#FFE600] hover:text-white">Ver</a>
                </div>
                @empty
                <p class="text-white/70 text-center py-4">No hay contactos registrados</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="panel-card-dark p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="p-4 rounded-xl bg-white/5 flex justify-between items-start panel-card-dark__row hover:bg-white/8">
                    ...
                    <a href="{{ route('user.sales.show', $sale) }}" class="text-[#FFE600] hover:text-white">Ver</a>
                </div>
                @empty
                <p class="text-white/70 text-center py-4">No hay ventas registradas</p>
                @endforelse
            </div>
        </div>
        @endcan
        ...
        <div class="panel-card-dark p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="p-4 rounded-xl bg-white/5 border-l-4 @if($followUp->completado) border-l-[#15803D] @elseif($followUp->estaVencido()) border-l-[#B91C1C] @else border-l-[#B45309] @endif">
                    <span class="px-2.5 py-1 text-xs font-medium rounded-lg badge-followup-{{ $followUp->completado ? 'completado' : ($followUp->estaVencido() ? 'vencido' : 'pendiente') }}">{{ ucfirst($followUp->tipo_accion) }}</span>
                    <p class="text-sm text-white/80 mt-2">{{ $followUp->fecha_alarma->format('d/m/Y H:i') }}</p>
                    @if($followUp->bitacora_notas)<p class="text-sm text-white mt-2">{{ $followUp->bitacora_notas }}</p>@endif
                </div>
                @empty
                <p class="text-white/70 text-center py-4">No hay seguimientos registrados</p>
                @endforelse
            </div>
        </div>
    </div>
