@php
    ...
    $emptyExtra = null;
    if ($sales->isEmpty() && auth()->user()->can('sales.create')) {
        if ($scope === 'company') {
        ...
        } elseif ($scope === 'contact' && $contact) {
            $emptyExtra = '<a href="' . e(route('user.sales.create', ['company_id' => $company->id, 'contact_id' => $contact->id])) . '" class="text-[#FFE600] hover:text-white underline ml-1">Registrar venta para este contacto</a>';
        }
    }
@endphp
<x-app-user-layout>
