...
@php
    $emailPrimario = trim((string) ($executive->email ?? ''));
    ...
    >
        ...
        >
            <div class="px-5 py-3 bg-[#FFE600] flex items-center justify-between text-[#003366] rounded-t-2xl">
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs opacity-90 truncate">{{ $executive->name }}</p>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('reminders.store') }}" class="px-5 py-4 space-y-4 text-white overflow-y-auto flex-1">
                ...
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-1.5">Titulo del recordatorio</label>
                    <input type="text" name="title" required maxlength="255" value="{{ old('title') }}" style="color: #111827;" class="w-full rounded-xl border border-gray-300 bg-white placeholder-gray-400 focus:ring-2 focus:ring-[#FFE600]/60 py-2.5 px-3 text-sm" placeholder="Ej. Seguimiento con {{ Str::limit($executive->name, 40) }}">
                    @error('title')
                ...
                <div class="pt-4 border-t border-white/15">
                    <p class="text-xs font-semibold uppercase tracking-wider text-[#FFE600]/90 mb-3">Datos del ejecutivo (puedes editarlos)</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-medium text-white/90 mb-1">Nombre del cliente</label>
                            <input type="text" name="nombre_cliente" maxlength="255" value="{{ old('nombre_cliente', $executive->name) }}" style="color: #111827;" class="w-full rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-[#FFE600]/60 py-2.5 px-3 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-white/90 mb-1">Empresa</label>
                            <input type="text" name="empresa" maxlength="255" value="{{ old('empresa', $companyName) }}" style="color: #111827;" class="w-full rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-[#FFE600]/60 py-2.5 px-3 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-white/90 mb-1">Correo electronico</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-white/90 mb-1">Numero telefonico</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-white/90 mb-1">Area</label>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="puesto_trabajo" maxlength="255" value="{{ old('puesto_trabajo', 'EJECUTIVO') }}" style="color: #111827;" class="w-full rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-[#FFE600]/60 py-2.5 px-3 text-sm">
                        </div>
                    </div>
                </div>
