<x-app-layout>
    ...
    <div class="space-y-8">
        ...
        @if(isset($contacts) && $contacts->isNotEmpty())
            <div class="panel-card-dark overflow-hidden border border-white/12 ring-1 ring-[#FFE600]/10 shadow-xl shadow-black/30">
                ...
                <div class="crm-table-scroll-wrap crm-table-scroll crm-table-scroll-wrap--scrollbar-top w-full min-w-0 -mx-1 px-1">
                    <div class="crm-table-scroll-wrap__flip-inner min-w-0">
                    <table class="w-full min-w-[920px] table-fixed divide-y divide-white/15">
                        <thead>
                            <tr class="table-header-panel-dark">
                                ...
                                <th class="w-[19%] min-w-0 px-4 py-3.5 text-left text-xs font-semibold uppercase">Nombre</th>
                                <th class="w-[21%] min-w-0 px-4 py-3.5 text-left text-xs font-semibold uppercase">Empresa</th>
                                <th class="w-[17%] min-w-[10rem] px-4 py-3.5 text-left text-xs font-semibold uppercase">Tel / Cel</th>
                                <th class="w-[11%] min-w-0 max-w-[11rem] px-4 py-3.5 text-left text-xs font-semibold uppercase">Email</th>
                                <th class="w-[17%] min-w-0 px-4 py-3.5 text-left text-xs font-semibold uppercase">Ejecutivo</th>
                                <th class="w-[12%] min-w-0 px-4 py-3.5 text-left text-xs font-semibold uppercase">Estatus</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-white/10">
                            @foreach($contacts as $contact)
                                <tr class="panel-card-dark__row odd:bg-white/5 even:bg-white/[0.02] hover:bg-white/10 transition-colors duration-150">
                                    ...
                                    <td class="w-[19%] min-w-0 px-4 py-3.5 text-sm align-top whitespace-normal break-words">
                                    ...
                                    <td class="w-[21%] min-w-0 px-4 py-3.5 text-sm text-white/90 align-top whitespace-normal break-words">
                                        @if($contact->company)
                                    ...
                                    <td class="w-[17%] min-w-[10rem] px-4 py-3.5 text-sm text-white/90 align-top tabular-nums">{{ $contact->telefono ?? $contact->celular ?? '—' }}</td>
                                    <td class="w-[11%] min-w-0 max-w-[11rem] px-4 py-3.5 text-sm text-white/90 align-top break-words" title="{{ $contact->email ?? '' }}">{{ $contact->email ?? '—' }}</td>
                                    <td class="w-[17%] min-w-0 px-4 py-3.5 text-sm text-white/90 align-top">{{ $contact->comercialEjecutivoLabel() }}</td>
                                    <td class="w-[12%] min-w-0 px-4 py-3.5 align-top"><span class="px-2 py-0.5 text-xs rounded-lg badge-prospect-{{ $contact->status_color ?? 'seguimiento' }}">{{ $contact->status_label ?? '—' }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @if(isset($companies) && $companies->isNotEmpty())
            <div class="panel-card-dark overflow-hidden border border-white/12 ring-1 ring-[#FFE600]/10 shadow-xl shadow-black/30">
                ...
                <div class="crm-table-scroll-wrap crm-table-scroll w-full min-w-0 -mx-1 px-1">
                    <table class="w-full min-w-[800px] table-fixed divide-y divide-white/15">
                        <thead>
                            <tr class="table-header-panel-dark">
                                <th scope="col" class="crm-row-marker-head w-11 min-w-[2.75rem] px-1 py-3.5 text-center text-[10px] font-semibold uppercase tracking-wide text-[#FFE600]/90" title="Seguimiento personal (solo en este navegador)">Seg.</th>
                                <th class="w-[30%] min-w-0 max-w-[22rem] px-4 py-3.5 text-left text-xs font-semibold uppercase">Nombre</th>
                                <th class="w-[15%] min-w-[7.5rem] px-4 py-3.5 text-left text-xs font-semibold uppercase">RFC</th>
                                <th class="w-[22%] min-w-[10rem] px-4 py-3.5 text-left text-xs font-semibold uppercase">Estatus</th>
                                <th class="w-[33%] min-w-0 px-4 py-3.5 text-left text-xs font-semibold uppercase">Ejecutivo</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-white/10">
                            @foreach($companies as $company)
                                <tr class="panel-card-dark__row odd:bg-white/5 even:bg-white/[0.02] hover:bg-white/10 transition-colors duration-150">
                                    ...
                                    <td class="w-[30%] min-w-0 max-w-[22rem] px-4 py-3.5 text-sm align-top whitespace-normal break-words">
                                    ...
                                    <td class="w-[15%] min-w-[7.5rem] px-4 py-3.5 text-sm text-white/90 align-top">{{ $company->rfc ?? '—' }}</td>
                                    <td class="w-[22%] min-w-[10rem] px-4 py-3.5 align-top"><x-company-prospect-status-badges :company="$company" variant="filtros" /></td>
                                    <td class="w-[33%] min-w-0 px-4 py-3.5 text-sm text-white/90 align-top">{{ $company->assignedExecutive?->name ?? $company->ejecutivo_asignado ?? '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    <script>
        (function () {
            ...
            /** Texto libre guardado como E:base64 (ej. ejecutivo_asignado en empresas). */
            function decodeComercialLegacyKey(key) {
                if (!key.startsWith('E:')) {
                    return key;
                }
                const b64 = key.slice(2);
                try {
                    const bin = atob(b64);
            ...
            function displayValueForFilterField(field, raw) {
                ...
                if (field === 'comercial') {
                    ...
                    if (key.startsWith('E:')) {
                        return decodeComercialLegacyKey(key);
                    }
                    return key;
                }
                return key;
            ...
            function renderSummary() {
                ...
                entries.forEach(([field, values]) => {
                    ...
                    const displayVals = values.map((v) => displayValueForFilterField(field, v));
